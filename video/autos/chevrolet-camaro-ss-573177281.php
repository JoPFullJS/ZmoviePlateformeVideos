...
//}
  ...
  // mysql_select_db(DB_PROMO,$promo_bd);
   ...
   $req_keyword = $pdo->query($select_keyword);
   $verif_keyword = $req_keyword->fetch();
   ...
   $keyword=explode(',',$mot);
	...
	$req_similaire = $pdo->query($select_similaire);

   
   
   //On selectionne les element de la categorie
   $video_select = "SELECT a.ID_element,a.ID,a.ID_categorie,a.titre,a.description,a.ID_data,a.date,a.keyword,b.ID_lien,b.lk_image,b.lk_fichier,b.embed_video,c.id_stat,c.nb_vue,d.ID_compteur,d.vt_pos,d.vt_neg FROM descriptions AS a,lien AS b,statistiques AS c,compteur AS d WHERE a.ID_categorie='".$categorie."' AND a.ID_element=b.ID_lien AND a.ID_element=c.id_stat AND a.ID_element=d.ID_compteur AND ID_element IN ('".$id."')";
   ...
   $req_id = $pdo->query($select_id);
   $verif_id = $req_id->fetch();
   $nb_id = $verif_id['id'];
   ...
   //Pour toutes les videos
		...
		if($id_courant == $nb_id)
		{
			...
			$req_sui = $pdo->query($select_sui);
			$verif_sui = $req_sui->fetch();
		}
		else
		{
		...
		if($id_courant == 1)
		{
			...
			$req_pre = $pdo->query($select_pre);
			$verif_pre = $req_pre->fetch();
		}
		else
		{
   ...
   <!-- contenu -->
	...
	<div class="float_nav">
		...
		<div  class="gris_line"></div>
			...
			<ul class="box_caster">
		 <?php while($verif_similaire = $req_similaire->fetch()) { ?>
		   ...
		   if(isset($rand_650))
		   {
			while($verif_650 = fetch($req_select_650)) 
				{
	...
	<div class="like_reseau_sociaux">
	   ...
	   if(isset($rand_350))
	   {
		while($verif_350 = fetch($req_select_350)) 
			{
