...
//categorie
   ...
   $count_cat = "SELECT COUNT(DISTINCT ID_element) AS id FROM descriptions WHERE ID_categorie='".$categorie."'";
   ...
   if(isset($_GET['page']))
   {
	 ...
	 if($_GET['page'] > ceil($verif_cat['id']/18))
	 {
	  $ID_courant = ceil($verif_cat['id']/18);
	 }
   }
   else
   {
   ...
   //Info meta et lin commun
   $select_meta = "SELECT cat.ID,cat.theme,cat.description,cat.keyword FROM categories AS cat WHERE ID='".$categorie."'";
   $req_meta = mysql_query($select_meta,$rent_bd);
   //On selectionne les element de la categorie
   $cat_select = "SELECT a.ID_element,a.ID_categorie,a.ID_data,a.titre,a.date,b.ID_lien,b.lk_image,b.lk_fichier,c.id_stat,c.nb_vue,d.ID_compteur,d.vt_pos,d.vt_neg FROM descriptions AS a,lien AS b,statistiques AS c,compteur AS d WHERE a.ID_categorie='".$categorie."' AND a.ID_element=b.ID_lien AND a.ID_element=c.id_stat AND a.ID_element=d.ID_compteur AND d.vt_pos NOT IN ('1') AND d.vt_neg NOT IN ('0') ORDER BY ((d.vt_pos*100)/(d.vt_pos+d.vt_neg)) DESC LIMIT ".$lim_bas.",".$lim_haut."";
...
<?php $verif_meta = mysql_fetch_array($req_meta); ?>
<meta name="keywords" content="<?php echo $verif_meta['keyword']; ?>" />
<meta name="description" lang="fr" content="<?php echo $verif_meta['description']; ?>" />
<link rel="canonical" href="http://www.divertiz.com/video/<?php echo $base; ?>.php" />
<link rel="shortcut icon" href="http://divertiz.com/img/img_main/favicon.ico">
...
<title><?php echo $verif_meta['description']; ?></title>
...
<!-- END ExoClick.com Ad Code -->
   ...
   <!-- contenu -->
	...
	<div class="float_nav">
			...
			<span><a class="txt_li_nav" href="http://www.divertiz.com/pictures/All_pictures.php">pictures</a></span>
			<span class="txt_nav"> &rsaquo; </span>
			<span><a class="txt_li_pos" href="http://www.divertiz.com/pictures/<?php echo $verif_meta['theme']; ?>.php"><?php echo $verif_meta['theme']; ?></a></span>
	</div>
		<div class="titre_categorie"><h4 style="margin-top:0px; margin-bottom:0px;"><span style="text-transform : capitalize;"><?php echo $verif_meta['theme']; ?></span><span> : Meilleurs classement</span></h4></div>
		<div  class="red_line"></div>
		<div class="pertinance">
		 <ul class="pertinancepos">
		<li class="cast_pert"><a class="connectpert lkpert" href="<?php echo $verif_meta['theme']; ?>.php">New videos</a></li>
		<li class="cast_pert"><a class="connectpert lkpert" href="<?php echo $verif_meta['theme']; ?>/top_votes.php">Meilleurs classement</a></li>
		<li class="cast_pert"><a class="connectpert lkpert" href="<?php echo $verif_meta['theme']; ?>/top_vues.php">Les plus consultés</a></li>
	</ul>
	
		</div>
		<div class="info_pertinace">Pertinance : <span style="color:rgb(192,8,13); letter-spacing:1px;">Meilleurs classement</span><span></span></div>
...
{
	echo '<li class="page_number_nav float_left"><a href=http://www.divertiz.com/pictures/'.$catego.'/top_votes.php?page=1>Debut&nbsp;</a></li>';
}
...
{
	echo '<li class="page_number_nav float_left" style="margin-right:5px;"><a href=http://www.divertiz.com/pictures/'.$catego.'/top_votes.php?page='.$prev.'>Prec.&nbsp;</a></li>';
}
/////////////////////////////////
if($ID_courant >= $nb_page-2)
{
  if($fin>0)
  {
	...
	{
	  echo '<li class="page_number float_left"><a href=http://www.divertiz.com/pictures/'.$catego.'/top_votes.php?page='.$page_m.'>'.$page_m.'</a></li>';
	}
  }
  else
  {
...
if($ID_courant <=3)
{
  if($debut<$nb_page)
  {
	for($page_p=$ID_courant+1;$page_p<=$debut;$page_p++)
	{
	  ...
	  echo '<li class="page_number float_left"><a href=http://www.divertiz.com/pictures/'.$catego.'/top_votes.php?page='.$page_p.'>'.$page_p.'</a></li>';
	}
  }
  else
  {
...
{
	echo '<li class="page_number_nav float_left" style="margin-left:5px;"><a href=http://www.divertiz.com/pictures/'.$catego.'/top_votes.php?page='.$suiv.'>Suiv.&nbsp;</a></li>';
}
...
{
	echo '<li class="page_number_nav float_left"><a href=http://www.divertiz.com/pictures/'.$catego.'/top_votes.php?page='.$nb_page.'>Fin&nbsp;</a></li>';
}
?>
</ul>
</div>
